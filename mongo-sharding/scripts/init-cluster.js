...
for (const shard of [
    {name: "shard1RS", host: "shard1RS/shard1-1:27018"},
    {name: "shard2RS", host: "shard2RS/shard2-1:27018"},
]) {
    const current = config.shards.findOne({_id: shard.name});
    if (current) {
        assert.equal(current.host, shard.host, "Unexpected shard configuration");
    } else {
...
const result = application.helloDoc.bulkWrite(operations, {ordered: false});
