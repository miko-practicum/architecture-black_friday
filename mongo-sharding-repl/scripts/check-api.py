...
class ShardingAPI(unittest.TestCase):
    ...
    def test_replica_counts(self):
        for path in ("/", "/helloDoc/count"):
            _, response = request(path)
            self.assertEqual(response["replicas_per_shard"], {"shard1RS": 3, "shard2RS": 3})
            self.assertEqual(response["data_replicas_count"], 6)
            for shard in ("shard1", "shard2"):
                info = response["replica_sets"][f"{shard}RS"]
                self.assertEqual(info["replica_set"], f"{shard}RS")
                self.assertEqual(info["members"], [f"{shard}-{i}:27018" for i in range(1, 4)])
