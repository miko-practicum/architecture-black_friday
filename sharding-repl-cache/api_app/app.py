...
CACHE_PREFIX = f"api:cache:{DATABASE_NAME}:users:v1"
CACHE_TTL = 60
REDIS_UNAVAILABLE = (RedisConnectionError, RedisTimeoutError)
redis_client = None


def users_cache_key(collection_name: str) -> str:
    return f"{CACHE_PREFIX}:{quote(collection_name, safe='')}"


def users_key_builder(func, namespace, *, request, response, args, kwargs):
    return users_cache_key(kwargs["collection_name"])


class OptionalRedisBackend(RedisBackend):
    """Keep MongoDB reads available during Redis connection failures."""

    async def get_with_ttl(self, key):
        try:
            return await super().get_with_ttl(key)
        except REDIS_UNAVAILABLE:
            logger.warning("Redis unavailable; reading from MongoDB", exc_info=True)
            return 0, None
    ...
    async def set(self, key, value, expire=None):
        try:
            ...
            logger.warning("Redis unavailable; cache fill skipped", exc_info=True)


async def invalidate_users(collection_name: str):
    if redis_client is not None:
        try:
            await redis_client.delete(users_cache_key(collection_name))
        except REDIS_UNAVAILABLE:
...
async def startup():
    ...
    if REDIS_URL:
        ...
        FastAPICache.init(OptionalRedisBackend(redis_client), prefix=CACHE_PREFIX)
...
@cache(expire=CACHE_TTL, key_builder=users_key_builder)
async def list_users(collection_name: str):
    """
    List all of the user data in the database.
    The response is unpaginated and limited to 1000 results.
    """
    # Preserve the original exercise's simulated latency without blocking the loop.
...
async def create_user(collection_name: str, user: UserModel = Body(...)):
    ...
    await invalidate_users(collection_name)
